...
ANNOT_SUBTYPES = set(['Text', 'Highlight', 'Squiggly', 'StrikeOut', 'Underline','Stamp'])
...
def getannots(pdfannots, pageno):
    ...
    for pa in pdfannots:
        ...
        if pa.get('ReaddleStampText') is not None:
            contents = pa.get('ReaddleStampText')
...
def lambda_handler(event,context={}):
    ...
    return printannots(buffer)

def main():
    if len(sys.argv) == 4:
        print(lambda_handler({ 'bucket': sys.argv[2], 'key': sys.argv[3] }));
        return
